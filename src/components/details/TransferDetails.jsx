  ...
  return (
    <>
      <div style={s.section}>
        ...
        {isHandOff ? (
            <>
            <div style={s.row}><span style={s.label}>Target Flow</span><span style={s.value}>{details.handOffFlow?.handOffToName}</span></div>
            <div style={s.row}><span style={s.label}>Flow ID</span><span style={s.value}>{details.handOffFlow?.handOffTo}</span></div>
            </>
        ) : (
            <div style={s.row}><span style={s.label}>Destination Number</span><span style={s.value}>{details.transfertodn || details['transfertodn:name']}</span></div>
        )}
      </div>
    </>
  );
